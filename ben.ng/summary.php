...
$neighborhood_count = $array_count($restaurants, function($carry, $restaurant) {
	preg_match("/([0-9]+)(er|ème)/", $restaurant->neighborhood, $m);
	if(isset($carry))
		$carry[$m[1]]++;
	else
		$carry[$m[1]] = 1;
...
foreach($restaurants as $restaurant) {
	if(preg_match("/$arrondissement(er|ème)/", $restaurant->neighborhood)) {
		...
		$coordinates_tostring .= $restaurant->longitude . ',' . $restaurant->latitude . '|';
...
<img src="http://maps.googleapis.com/maps/api/staticmap?center=<?=$center["longitude"]?>,<?=$center["latitude"]?>&zoom=14&size=1080x540&markers=color:blue|<?=$coordinates_tostring?>" />
...
?>
	<tr><td><a href="restaurant.php?name=<?=urlencode($rest_to_arro->name)?>"><button type="button" class="btn btn-success">View</button></a></td><td><?=$rest_to_arro->name?></td><td><?=$rest_to_arro->rating?></td><td><?=$rest_to_arro->neighborhood?></td><td><?=$rest_to_arro->address?></td></tr>
<?php
	}
?>
</tbody>
